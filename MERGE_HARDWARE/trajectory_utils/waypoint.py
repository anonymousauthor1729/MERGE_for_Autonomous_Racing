...
class WayPoint:
    def __init__(self, 
        ...
        self.waypoints_tck = [
            splrep(progress, self.waypoints[:, i], s=smoothness_params[i], per=1) 
        ...
        if plot_waypoints_flag:
            ...
            switch_axis = plot_waypoints(self.waypoints, self.waypoints_width, ax)
            if switch_axis: waypoints_tck = self.waypoints_tck[::-1]
            else: waypoints_tck = self.waypoints_tck

            ax.plot(
                splev(progress, waypoints_tck[0]), 
                splev(progress, waypoints_tck[1]), 
    ...
    def find_progress_value(self, car_pos: np.ndarray):
        ...
        if car_pos.ndim == 1:
            car_pos = car_pos[None, :] 
            single_input = True

        _, nearest_idx = self.waypoints_tree.query(car_pos)
        ...
        error_vec = car_pos - self.waypoints[nearest_idx]
    ...
    def compute_deviations(self, car_pos: np.ndarray):
        nearest_waypoints = self.find_progress_value(car_pos)[1]
        deviations = np.linalg.norm(nearest_waypoints - car_pos, axis=-1)
        return deviations
