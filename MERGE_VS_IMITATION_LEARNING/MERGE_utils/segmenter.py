...
class GMM_Segmentation_Handler(GaussianMixture):
    ...
    def preprocess_trajectories(self, TDList, progress_scaler:float=1e3):
        """
        Spatially aligns and normalizes a list of time-varying expert demos.
        ...
        self.num_t_steps = int(min(len(TD["states"]) for TD in TDList))
        ...
        dt = np.diff(t_steps, axis=1)[:, :, np.newaxis]  # shape match
        dX = np.diff(X_all, axis=1)

        X_all_dot_partial = np.zeros_like(dX)
        np.divide(dX, dt, out=X_all_dot_partial, where=dt != 0)
        ...
        self.progress_feature_factor = self.num_segs * np.max(deviations) * progress_scaler
    ...
    def fit_gmm_model(self, features, downsample:int=1):
        """
        ...
        """
        # Filter extremely low variance features to enhance clustering
        deviation_tol = np.percentile(np.abs(features[:, 1]), 80)
        features_ = features[np.abs(features[:, 1]) > deviation_tol]
        self.fit(features_[::downsample])
        del features_
    ...
    def blend_trajectories(self, X_all, X_all_dot, weights, best_indices):
        """
        ...
        X_blended = np.einsum(
            'tk,ktd->td', weights, X_all[best_indices]
        )
    ...
    def plot_gmm_results(self, 
        ...
        gs = GridSpec(4, 2, width_ratios=[1, 1], hspace=0.2, wspace=0.125)
        ...
        ax_xy.set_title(f"({chr(97)}) Segmented Track Map")
        ax_xy.axis("equal")

        # --- 2. VARIANCE ENVELOPE ---
        plot_index = 0
        ax_f = fig.add_subplot(gs[plot_index, 1])
        ax_f.scatter(features[:, 0], features[:, 1], 
                     alpha=0.25, c=feature_labels, cmap=custom_cmap, s=10)
        
        ax_f.set_title(f"({chr(97 + plot_index)}) Track Complexity (Lateral Deviations)")
        ax_f.set_ylabel("Deviation [m]")
        ...
        plot_index += 1
        ax_g = fig.add_subplot(gs[plot_index, 1])
        ...
        plot_index += 1
        ax_t = fig.add_subplot(gs[plot_index, 1])
        ...
        if best_indices is not None and cluster_scores is not None:
            ...
            cbar.set_label('Demo Score', rotation=270, labelpad=20)
            cbar.outline.set_visible(False)

        # --- 5. ACTIVATION WEIGHTS (gamma) ---
        plot_index += 1
        ax_w = fig.add_subplot(gs[plot_index, 1])
